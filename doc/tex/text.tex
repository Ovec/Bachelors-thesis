...
\n{3}{Operator install}
The installation of the operator was done with the following commands in the \break /tests/PGO/1\_operator\_install directory, that contains yaml manifests extracted from the PGO examples repository without any change.
...
To install the Postgres cluster, the following command must be executed in the install directory (/tests/PGO/1\_cluster\_install).
\begin{verbatim}
kubectl apply -k ./high-availability
\end{verbatim}

This command has installed the following pods and services in postgres-operator namespace
