...
\begin{itemize}
  ...
  \item Cluster install
        \begin{itemize}
          ...
          \item Creation of a database on the Primary node
          \item Synchronization between nodes check
        \end{itemize}
...
\n{2}{Crunchy Postgres for Kubernetes (PGO) tests}


\n{3}{Operator install}
The Crunchy documentation contains a Quickstart chapter, according to which the Operator will be installed. To install, it is necessary to have kubectl installed and set up for the cluster in where the PGO is to be installed.

The installation of the operator is done with the following commands in the \break /tests/PGO/1\_operator\_install directory, that contains yaml manifests that were extracted from the PGO examples repository \footnote{https://github.com/CrunchyData/postgres-operator-examples}.
...
These commands create the following kubernetes objects:
\begin{itemize}
...
To check that the Operator is created correctly, the following command is used:
\begin{verbatim}
kubectl -n postgres-operator get pods
\end{verbatim}

This command displays all pods from the postgres-operator namespace. If the Operator has been installed correctly the command will display the pod operator with the status Running.

The result of this command is the following:
\begin{verbatim}
NAME                           READY   STATUS    RESTARTS   AGE
pgo-864ddbd4cf-4pcd6           1/1     Running   0          4m6s
pgo-upgrade-78fc8d8f94-jbnrl   1/1     Running   0          4m6s
\end{verbatim}
